...
本文档是 Codex Aurora Skin 在 Windows、macOS、Linux 和共用模块上的统一开发清单。
...
平台使用 `ALL`、`WIN`、`MAC`、`LINUX` 标记。编号格式为
...
- 开始工作前将状态更新为 `进行中`；完成后记录提交或 PR、验证结果，再移入
  “已完成归档”。
- `ALL` 项必须完成 Windows、macOS 与 Linux 验证；历史版本在 Linux 首版进入
  支持范围前形成的验收记录仍按当时的平台范围解释。仅影响单个平台时，不要求
  为其他平台制造无关改动。
...
| `v1.0.1` | 已发布 | 处理 Codex 结构兼容、主页布局和 macOS 休眠恢复问题 |
| `v1.0.2` | 候选 | 修复 macOS 引擎降级、异常会话残留和诊断目标选择问题；同步三平台版本字段 |
| Linux 首版 | 进行中 | ChatGPT DEB/RPM 发现、CDP 会话、用户安装与发行构件 |
...
| `BUG-WIN-002` | Windows | P2 | 已完成 | `v1.0.1` 候选 | 升级安装时自动隐藏程序目录选择页 |
...
| `BUG-MAC-003` | macOS | P1 | 已完成 | `v1.0.2` 候选 | 旧构建覆盖已安装的新引擎，导致选择器和注入器回退 |
| `BUG-MAC-004` | macOS | P1 | 已完成 | `v1.0.2` 候选 | 一次性注入异常后遗留 CDP 会话和 `applying` 状态 |
| `BUG-MAC-005` | macOS | P2 | 已完成 | `v1.0.2` 候选 | selector doctor 优先选中辅助窗口并误报失败 |
...
### BUG-WIN-002：升级安装隐藏程序目录页

- 证据：Inno Setup 的 `DisableDirPage` 默认值为 `auto`；检测到相同 `AppId` 的
  已安装版本后，会自动跳过“选择目标位置”页。当前定义未显式覆盖该默认值。
- 修复：设置 `DisableDirPage=no`，让首次安装和升级始终显示目录页；显式保留
  `UsePreviousAppDir=yes`，升级时默认带出旧路径但允许用户修改。
- 边界：所选路径只存放启动器和程序文件。运行时状态、主题与用户数据继续位于
  `%LOCALAPPDATA%\CodexAuroraSkin`，本次不引入便携模式或数据迁移。
- 验收：安装器静态测试同时要求两个目录指令；PowerShell 7 与 Windows
  PowerShell 5.1 均通过安装器门禁并重新构建 v1.0.1 安装包。
...
### BUG-MAC-003：旧构建覆盖新引擎

- 证据：应用程序目录仍是 1.0.1 旧构建，而用户引擎曾更新到新版；每次启动旧 App
  都会无条件替换 `~/.codex/codex-aurora-skin`，把新版注入器和选择器降回旧文件。
- 修复：引擎加入独立递增构建号；1.0.2 起仅在安装不完整或捆绑版本更高时替换，
  并在构建阶段校验 App 与引擎构建号一致。
- 验收：安装 1.0.2 后再次启动同一 App，引擎目录时间和注入器哈希保持不变；
  已安装完整的新版本不得被同版本或更旧的捆绑引擎覆盖。

### BUG-MAC-004：一次性注入异常后会话残留

- 证据：主题载入或进入逐目标处理前失败时，已连接的 CDP 会话没有统一释放，
  可留下孤儿 `--once` 进程并让界面停在“正在应用皮肤”。
- 修复：一次性流程增加覆盖全部异常出口的统一清理边界。
- 验收：模拟准备阶段异常时，每个已连接会话均关闭；实机重新应用后仅保留一个
  管理器和一个长期 watcher，状态进入 `active`。

### BUG-MAC-005：诊断工具选错辅助窗口

- 证据：多个 `app://` 页面并存时，doctor 按接口返回顺序选择首项，可能命中
  detached/avatar 辅助窗口并报告主结构缺失。
- 修复：优先选择精确的 `app://-/index.html` 主页面，再考虑带查询参数的主页面。
- 验收：混合目标单元测试稳定选择主页面；实机 doctor 的 L1 结构完整且
  `missingL1=[]`。
...
| 编号 | 平台 | 优先级 | 状态 | 目标版本 | 摘要 |
| --- | --- | --- | --- | --- | --- |
| `DEV-LINUX-001` | Linux | P1 | 进行中 | Linux 首版 | 支持官方 ChatGPT Linux 桌面端换肤、恢复和安装构件 |

### DEV-LINUX-001：Linux 平台支持

- 范围：Ubuntu/Debian 的 `chatgpt` DEB、Fedora 的 `chatgpt` RPM，以及 x64、
  ARM64 架构。
- 安全边界：不修改官方应用文件；校验包归属、摘要、架构、随包 Node.js、进程
  可执行路径和回环 CDP 所有权；不安装自启动服务。
- 构件：用户级安装脚本、TAR、DEB 与 RPM 构建配置。
- 验收：共享回归、Linux 静态契约、真实注入/热更新/恢复、DEB 内容检查通过；
  Fedora、ARM64 和官方支持发行版的最终验收在对应环境完成。
- 当前开发机证据：Ubuntu 22.04 x64（非 OpenAI 正式支持版本）、ChatGPT
  `26.814.41407` DEB、随包 Node.js `v24.19.0` 已完成真实注入与 DOM 显示校验；
  恢复后状态文件、注入器和回环 CDP 监听均消失，ChatGPT 以无调试参数方式启动。
- 待完成发布矩阵：Ubuntu 24.04/26.04、Debian 13、Fedora 43/44、ARM64，
  以及 Fedora 环境中的 RPM 二进制构建和安装验收。
