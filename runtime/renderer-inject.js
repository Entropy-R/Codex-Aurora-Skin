...
((cssText, artDataUrl, themeConfig) => {
  const SELECTOR_CONTRACT = __AURORA_SKIN_SELECTORS_JSON__;
  ...
  const LOCAL_MARKERS = {
    searchBand: "data-dream-search-band",
    searchInput: "data-dream-search-input",
    projectHost: "data-dream-project-host",
  };
  const NAVIGATION_RETRY_DELAYS = [64, 250, 1000, 2500];
  const VERSION = __AURORA_SKIN_VERSION_JSON__;
  const STYLE_REVISION = __AURORA_SKIN_STYLE_REVISION_JSON__;
  const PAYLOAD_REVISION = __AURORA_SKIN_PAYLOAD_REVISION_JSON__;
  const THEME = themeConfig && typeof themeConfig === "object" ? themeConfig : {};
  const ART = THEME.art && typeof THEME.art === "object" ? THEME.art : {};
  const VISUAL = THEME.visual && typeof THEME.visual === "object" ? THEME.visual : {};
  const ART_METADATA = THEME.artMetadata && typeof THEME.artMetadata === "object"
    ? THEME.artMetadata : null;
  const ANALYSIS_CACHE_KEY = "__CODEX_AURORA_SKIN_ANALYSIS_CACHE__";
  const THEME_VARIABLES = [
    ...
    "--ds-art-brightness", "--ds-art-overlay-opacity", "--ds-surface-opacity",
    "--ds-art-darken-opacity", "--ds-art-lighten-opacity",
    "--dream-art-focus-x", "--dream-art-focus-y", "--dream-art-position",
    "--aurora-skin-focus-x", "--aurora-skin-focus-y", "--aurora-skin-art-position",
    "--aurora-skin-project-prefix", "--aurora-skin-project-label",
    "--aurora-skin-art",
  ];
  ...
  const applyTheme = (root, shell) => {
    ...
    // 图片参数只作用于背景层；界面底色强度只改变面板透明度，不影响文字和图标。
    const safeVisual = shell === "dark"
      ? { brightness: 0.62, overlayOpacity: 0.28, surfaceOpacity: 0.78 }
      : { brightness: 0.90, overlayOpacity: 0.32, surfaceOpacity: 0.65 };
    const requestedVisual = VISUAL[shell] && typeof VISUAL[shell] === "object"
      ? VISUAL[shell] : {};
    const brightnessValue = Number(requestedVisual.brightness);
    const overlayValue = Number(requestedVisual.overlayOpacity);
    const surfaceValue = Number(requestedVisual.surfaceOpacity);
    const brightness = clamp(
      Number.isFinite(brightnessValue) ? brightnessValue : safeVisual.brightness,
      0.35,
      1.20,
    );
    const overlayOpacity = clamp(
      Number.isFinite(overlayValue) ? overlayValue : safeVisual.overlayOpacity,
      shell === "light" ? 0.32 : 0,
      0.70,
    );
    const surfaceOpacity = clamp(
      Number.isFinite(surfaceValue) ? surfaceValue : safeVisual.surfaceOpacity,
      shell === "light" ? 0.60 : 0.20,
      1,
    );
    setStyleProperty(root, "--ds-art-brightness", brightness.toFixed(2));
    setStyleProperty(root, "--ds-art-overlay-opacity", overlayOpacity.toFixed(2));
    setStyleProperty(root, "--ds-surface-opacity", surfaceOpacity.toFixed(2));
    // body 背景无法单独使用 filter，宽图模式用等效的明暗混合层补偿。
    setStyleProperty(root, "--ds-art-darken-opacity", Math.max(0, 1 - brightness).toFixed(2));
    setStyleProperty(
      root,
      "--ds-art-lighten-opacity",
      Math.min(0.18, Math.max(0, brightness - 1) * 0.90).toFixed(2),
    );
    setStyleProperty(root, "--aurora-skin-project-prefix", cssString(THEME.projectPrefix || "选择项目 · "));
    setStyleProperty(root, "--aurora-skin-project-label", cssString(THEME.projectLabel || "◉  选择项目"));
  };
  ...
  const applyArtMetadata = (root) => {
    ...
    setStyleProperty(root, "--aurora-skin-focus-x", focusXValue);
    setStyleProperty(root, "--aurora-skin-focus-y", focusYValue);
    setStyleProperty(root, "--aurora-skin-art-position", `${focusXValue} ${focusYValue}`);
  };
    ...
    image.onload = () => {
      try {
        ...
        finish({
          ...
          ratio,
          // 与 aspect=wide 使用同一阈值，避免横图被错误降级为局部背景。
          wide: ratio >= 1.45,
          aspect,
          brightness,
  ...
  const applyRootState = (root) => {
    ...
    setAttribute(root, "data-aurora-skin", "active");
    setAttribute(root, SHELL_ATTR, shell);
    setStyleProperty(root, "--aurora-skin-art", `url("${artUrl}")`);
  ...
  const reconcileMarker = (attribute, nextNodes) => {
    ...
    markedNodes.set(attribute, next);
  };

  const refreshLocalMarkers = (baseState) => {
    const searchBands = new Set();
    const searchInputs = new Set();
    if (baseState !== "home") {
      for (const input of queryAll('input[type="text"]')) {
        const directHost = input.parentElement;
        if (directHost?.matches?.("div.no-drag")) searchInputs.add(directHost);
        const stickyHost = input.closest?.("div.sticky");
        if (stickyHost) searchBands.add(stickyHost);
      }
    }

    const projectHosts = new Set();
    if (baseState === "home") {
      const projectSelector = selectorByKey.get("project-selector")?.selector;
      for (const project of projectSelector ? queryAll(projectSelector) : []) {
        const fadeMask = project.closest?.(".horizontal-scroll-fade-mask");
        const host = fadeMask?.parentElement;
        if (host?.matches?.("div")) projectHosts.add(host);
      }
    }

    reconcileMarker(LOCAL_MARKERS.searchBand, searchBands);
    reconcileMarker(LOCAL_MARKERS.searchInput, searchInputs);
    reconcileMarker(LOCAL_MARKERS.projectHost, projectHosts);
  };
  ...
  const cleanup = () => {
    ...
    for (const timer of navigationRetryTimers) clearTimeout(timer);
    navigationRetryTimers.clear();
    navigationGeneration += 1;
    for (const [attribute, nodes] of markedNodes) {
  ...
  const navigationHandler = navigationApi ? () => {
    ...
    for (const timer of navigationRetryTimers) clearTimeout(timer);
    navigationRetryTimers.clear();
    for (const delay of NAVIGATION_RETRY_DELAYS) {
      const timer = setTimeout(() => {
        navigationRetryTimers.delete(timer);
        const state = window[STATE_KEY];
        if (state?.installToken !== installToken || window[DISABLED_KEY] ||
          generation !== navigationGeneration) return;
        ensure({ root: false, scope: true });
      }, delay);
      navigationRetryTimers.add(timer);
    }
  } : null;
...
})(__AURORA_SKIN_CSS_JSON__, __AURORA_SKIN_ART_JSON__, __AURORA_SKIN_THEME_JSON__)
