...
function pageCapture(CFG, PROBE_LIST) {
  ...
  var SKIN_NODE_ID = {
    "codex-aurora-skin-style": true,
    "codex-aurora-skin-chrome": true,
    "chatgpt-aurora-skin-operation": true,
  };
  var SKIN_CLASS = /^(?:dream-|codex-aurora-skin(?:-|$))/;
  ...
  var skinState = window.__CODEX_AURORA_SKIN_STATE__;
  var pathname = "";
  ...
  return {
    ...
    skin: {
      ...
      disabled: window.__CODEX_AURORA_SKIN_DISABLED__ === true,
...
function pageSignature(PROBE_LIST) {
  ...
  try {
    rootClasses = Array.prototype.filter.call(document.documentElement.classList, function (cls) {
      return !/^(?:dream-|codex-aurora-skin(?:-|$))/.test(cls);
    }).sort().join(".");
  } catch (error) {}
...
async function stateFilePorts() {
  ...
  if (process.platform === "darwin") {
    ...
    const root = path.join(process.env.LOCALAPPDATA, "CodexAuroraSkin");
...
async function candidatePorts(options) {
  ...
  const envPort = Number(process.env.CODEX_AURORA_SKIN_PORT);
