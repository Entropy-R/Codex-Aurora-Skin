...
if (contract.schema !== "codex-aurora-skin-selectors/1" || !Array.isArray(contract.selectors)) {
...
function compileRuntime(source) {
  const token = "__AURORA_SKIN_SELECTORS_JSON__";
...
const sourceCss = await fs.readFile(path.join(projectRoot, "runtime", "aurora-skin.css"), "utf8");
...
const outputs = [
  ...
  {
    content: compileSelectorTokens(sourceCss, "runtime/aurora-skin.css"),
    paths: ["macos/assets/aurora-skin.css", "windows/assets/aurora-skin.css"],
  },
  {
