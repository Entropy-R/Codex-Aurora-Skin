...
try {
  ...
  $currentCodex = $null
  try { $currentCodex = Get-AuroraSkinCodexInstall } catch { Write-Warning $_.Exception.Message }
  $savedPathCandidate = Get-AuroraSkinCodexStatePathCandidate -State $state
  ...
  $currentRunning = $null -ne $currentCodex -and (Get-AuroraSkinCodexProcesses -Codex $currentCodex).Count -gt 0
  $savedRunning = $null -ne $savedCodex -and (Get-AuroraSkinCodexProcesses -Codex $savedCodex).Count -gt 0
  $savedOwnsPort = $null -ne $savedCodex -and (Test-AuroraSkinCodexPortOwner -Port $Port -Codex $savedCodex)
  ...
  $codexRunning = $null -ne $codex -and (Get-AuroraSkinCodexProcesses -Codex $codex).Count -gt 0
  $portOwnedByCodex = $null -ne $codex -and (Test-AuroraSkinCodexPortOwner -Port $Port -Codex $codex)
  ...
  if ($null -ne $state -and $null -eq $codex -and -not (Test-AuroraSkinPortAvailable -Port $Port)) {
  ...
  if ($shouldCloseCodex -and $PromptRestart) {
    $restartMessage = if ($NoRelaunch) {
      'Restore will close Codex and remove Aurora Skin plus its CDP session. Continue?'
    } else {
      'Restore will close Codex, remove Aurora Skin and its CDP session, then reopen the official app. Continue?'
    }
    $forceAuthorized = Confirm-AuroraSkinRestart -Message $restartMessage
  ...
  $backup = Join-Path $StateRoot 'config.before-aurora-skin.toml'
  $config = Join-Path $HOME '.codex\config.toml'
  if ($RecoverConfigBackup) {
    if (-not (Test-Path -LiteralPath $backup)) { throw 'No pre-install config backup is available.' }
    $null = Read-AuroraSkinUtf8File -Path $backup
  } elseif ($RestoreBaseTheme) {
    if (-not (Test-Path -LiteralPath $backup)) { throw 'No pre-install config backup is available.' }
    $null = Read-AuroraSkinUtf8File -Path $backup
    $null = Read-AuroraSkinUtf8File -Path $config
  }

  $restoreError = $null
  try {
    if ($shouldCloseCodex) {
      Stop-AuroraSkinCodex -Codex $codex -AllowForce:$forceAuthorized
      if ($portOwnedByCodex -and -not (Wait-AuroraSkinPortAvailable -Port $Port -TimeoutSeconds 5)) {
        throw "Port $Port is still listening after Codex closed; state was preserved for inspection."
      }
    }

    $recordedInjectorStopped = Stop-AuroraSkinRecordedInjector -State $state
    ...
    if ($RecoverConfigBackup) {
      ...
      Restore-AuroraSkinConfigBackup -ConfigPath $config -BackupPath $backup -RecoveryBackupPath $recoveryBackup
      Write-Host "Recovered the exact pre-install config; previous current config saved at $recoveryBackup"
    } elseif ($RestoreBaseTheme) {
      Restore-AuroraSkinBaseTheme -ConfigPath $config -BackupPath $backup
    }
    if ($RecoverConfigBackup -or $RestoreBaseTheme) {
      ...
      Archive-AuroraSkinConfigBackup -BackupPath $backup -ArchivePath $archivePath
    ...
    if ($Uninstall) {
      ...
      @(
        (Join-Path $desktop 'Codex Aurora Skin.lnk'),
        (Join-Path $desktop 'Codex Aurora Skin - Restore.lnk'),
        (Join-Path $desktop 'Codex Aurora Skin - Tray.lnk'),
        (Join-Path $startMenu 'Codex Aurora Skin.lnk'),
        (Join-Path $startMenu 'Codex Aurora Skin - Tray.lnk')
    ...
    if ($shouldCloseCodex -and -not $NoRelaunch) {
      ...
      $null = Start-AuroraSkinCodex -Codex $relaunchCodex
    }
  } catch {
    $restoreError = $_
    if ($shouldCloseCodex -and -not $NoRelaunch -and $null -ne $relaunchCodex -and
      (Get-AuroraSkinCodexProcesses -Codex $codex).Count -eq 0 -and (Test-Path -LiteralPath $relaunchCodex.Executable)) {
      try { $null = Start-AuroraSkinCodex -Codex $relaunchCodex } catch {
  ...
  Write-Host 'Aurora Skin restore actions completed; any saved CDP session was closed.'
} finally {
