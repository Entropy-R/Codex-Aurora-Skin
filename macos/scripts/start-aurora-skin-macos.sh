...
record_start_exit() {
  ...
  printf 'ChatGPT Aurora Skin: start failed at line %s (exit %s). See %s\n' "$line" "$code" "$START_ERROR_LOG" >&2
}
...
if codex_is_running && [ "$DEBUG_READY" = "false" ]; then
  if [ "$PROMPT_RESTART" = "true" ] && [ "$RESTART_EXISTING" = "false" ]; then
    if ! /usr/bin/osascript -e 'display dialog "ChatGPT 需要重启一次才能启用皮肤。通常会在 10–30 秒内完成。" buttons {"取消", "重启并应用"} default button "重启并应用" with title "ChatGPT Aurora Skin"' >/dev/null; then
...
VERIFY_OUTPUT="$(/usr/bin/mktemp "${TMPDIR:-/tmp}/aurora-skin-verify.XXXXXX")"
...
OPERATION_FINISHED="true"
printf 'ChatGPT Aurora Skin %s is active on loopback port %s.\n' "$SKIN_VERSION" "$PORT"
