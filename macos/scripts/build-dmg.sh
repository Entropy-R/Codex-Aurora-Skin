...
TMP="$(/usr/bin/mktemp -d /tmp/codex-aurora-skin-dmg.XXXXXX)"
MOUNT=""
cleanup() {
...
/usr/bin/ditto "$APP" "$STAGE/Codex Aurora Skin.app"
...
[ -f "$MOUNTED_APP/Contents/Resources/engine/library/preset-red-white-abstract/theme.json" ] \
...
MOUNTED_ARCHS="$(/usr/bin/lipo -archs "$MOUNTED_APP/Contents/MacOS/CodexAuroraSkinMenuBar")"
read -r -a EXPECTED_ARCHS <<< "${AURORASKIN_ARCHS:-arm64 x86_64}"
