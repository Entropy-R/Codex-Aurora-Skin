...
if [ "$UNINSTALL" = "true" ] && [ ! -e "$STATE_PATH" ] &&
  ...
  if [ ! -e "$THEME_BACKUP_PATH" ]; then
    printf 'No active Aurora Skin session or config backup was found; safe engine-only cleanup.\n'
    exit 0
  fi
...
if [ "$UNINSTALL" = "true" ]; then
  for launcher in \
    "$HOME/Desktop/Codex Aurora Skin.command" \
    "$HOME/Desktop/Codex Aurora Skin - Customize.command" \
    "$HOME/Desktop/Codex Aurora Skin - Verify.command" \
    "$HOME/Desktop/Codex Aurora Skin - Restore.command"; do
    if [ -f "$launcher" ] && [ ! -L "$launcher" ] &&
       /usr/bin/grep -F -q '# CodexAuroraSkin launcher' "$launcher"; then
      /bin/rm -f "$launcher"
    fi
  done
fi

printf 'ChatGPT Aurora Skin was removed and the requested macOS restore actions completed successfully.\n'
