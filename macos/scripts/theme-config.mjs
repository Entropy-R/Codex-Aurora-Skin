...
const [mode, configPath, backupPath, appearanceArg, archivePath] = process.argv.slice(2);
...
if (!["install", "restore", "migrate"].includes(mode) || !configPath || !backupPath ||
    (mode === "migrate" && !archivePath)) {
  throw new Error("Usage: theme-config.mjs <install|restore|migrate> <config-path> <backup-path> [appearance] [archive-path]");
}
...
async function acquireConfigLock() {
  const lockPath = `${configPath}.aurora-skin.lock`;
  ...
  while (true) {
    ...
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error("Another Aurora Skin config operation is still running; try again shortly.");
      }
...
async function main() {
  ...
  validateBackup(backup);
  if (mode === "migrate") {
    if (section && (appearance === "light" || appearance === "dark")) {
      const { matches } = settingLines(section.body, "appearanceTheme");
      const managed = `appearanceTheme = "${appearance}"`;
      if (matches[0]?.trim() === managed) {
        const body = replaceSetting(
          section.body,
          "appearanceTheme",
          backup.values.appearanceTheme ?? null,
        );
    ...
    console.log("Archived the legacy theme backup after selective migration.");
    return;
  }
  if (!section) {
