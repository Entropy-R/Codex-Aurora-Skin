...
OUTPUT="${1:-$ROOT/menubar-app/Resources/AuroraSkin.icns}"
TMP="$(/usr/bin/mktemp -d /tmp/codex-aurora-skin-icon.XXXXXX)"
...
ICONSET="$TMP/AuroraSkin.iconset"
...
if [ -n "${AURORASKIN_SDK:-}" ]; then
  /usr/bin/xcrun swift -sdk "$AURORASKIN_SDK" \
