...
if [ -z "${HOME:-}" ]; then
  ...
  [ -n "$HOME" ] || { printf 'ChatGPT Aurora Skin: could not resolve the current macOS home directory.\n' >&2; exit 1; }
  export HOME
fi
...
INSTALL_ROOT="$HOME/.codex/codex-aurora-skin"
STATE_ROOT="$HOME/Library/Application Support/CodexAuroraSkin"
LEGACY_INSTALL_ROOT="$HOME/.codex/codex-dream-skin-studio"
LEGACY_STATE_ROOT="$HOME/Library/Application Support/CodexDreamSkinStudio"
...
CODEX_APP_JOB_LABEL="io.github.entropy-r.codex-aurora-skin.app"
INJECTOR_JOB_LABEL="io.github.entropy-r.codex-aurora-skin.injector"
...
SKIN_VERSION="1.0.1"
AURORA_SKIN_VALIDATED_RUNTIME_PID=""
AURORA_SKIN_VALIDATED_RUNTIME_BUNDLE=""
AURORA_SKIN_VALIDATED_RUNTIME_EXE=""
AURORA_SKIN_VALIDATED_RUNTIME_NODE=""
...
on run argv
  display notification (item 1 of argv) with title "ChatGPT Aurora Skin"
end run
APPLESCRIPT
}
...
alert_user() {
  ...
  /usr/bin/osascript - "$message" <<'APPLESCRIPT' >/dev/null 2>&1 || true
on run argv
  display alert "ChatGPT Aurora Skin" message (item 1 of argv)
end run
APPLESCRIPT
}
...
migrate_legacy_user_data() {
  [ -d "$LEGACY_STATE_ROOT" ] || return 0
  [ ! -e "$STATE_ROOT" ] || return 0

  local legacy_state="$LEGACY_STATE_ROOT/state.json"
  if [ -f "$legacy_state" ]; then
    ...
    legacy_pid="$("$NODE" -e '
      ...
      try {
        const value = JSON.parse(fs.readFileSync(process.argv[1], "utf8")).injectorPid;
        process.stdout.write(Number.isInteger(value) && value > 0 ? String(value) : "");
      } catch {}
    ' "$legacy_state")"
    if [ -n "$legacy_pid" ] && /bin/kill -0 "$legacy_pid" 2>/dev/null; then
      fail "The legacy Codex Dream Skin injector is still running. Restore and close it before migrating to Codex Aurora Skin."
    fi
  fi

  local legacy_link
  legacy_link="$(/usr/bin/find "$LEGACY_STATE_ROOT" -type l -print -quit 2>/dev/null || true)"
  [ -z "$legacy_link" ] || fail "Legacy theme data contains a symbolic link and cannot be migrated safely: $legacy_link"

  /bin/mkdir -p "$STATE_ROOT"
  /bin/chmod 700 "$STATE_ROOT"
  local entry
  for entry in theme themes images; do
    if [ -e "$LEGACY_STATE_ROOT/$entry" ] && [ ! -e "$STATE_ROOT/$entry" ]; then
      /usr/bin/ditto "$LEGACY_STATE_ROOT/$entry" "$STATE_ROOT/$entry"
    fi
  done
  if [ -f "$LEGACY_STATE_ROOT/overrides.json" ] && [ ! -e "$STATE_ROOT/overrides.json" ]; then
    /bin/cp "$LEGACY_STATE_ROOT/overrides.json" "$STATE_ROOT/overrides.json"
  fi
  # 保留旧版的官方外观基线，供新品牌恢复流程迁移并归档。
  if [ -f "$LEGACY_STATE_ROOT/theme-backup.json" ] && [ ! -e "$THEME_BACKUP_PATH" ]; then
    /bin/cp "$LEGACY_STATE_ROOT/theme-backup.json" "$THEME_BACKUP_PATH"
  fi
}
...
remember_validated_runtime_identity() {
  AURORA_SKIN_VALIDATED_RUNTIME_PID="$$"
  AURORA_SKIN_VALIDATED_RUNTIME_BUNDLE="$CODEX_BUNDLE"
  AURORA_SKIN_VALIDATED_RUNTIME_EXE="$CODEX_EXE"
  AURORA_SKIN_VALIDATED_RUNTIME_NODE="$NODE"
}
...
stop_recorded_injector() {
  ...
  if ! pid="$(state_field injectorPid 2>/dev/null)" || [ -z "${pid:-}" ]; then
    printf 'Aurora Skin state is damaged or missing its injector PID; state was preserved.\n' >&2
    return 1
  fi
  ...
  case "$pid" in
    *[!0-9]*|??????????*)
      printf 'Recorded Aurora Skin injector PID is invalid; state was preserved.\n' >&2
      return 1
      ;;
  esac
  ...
  case "$saved_port" in
    ''|*[!0-9]*)
      printf 'Recorded Aurora Skin injector port is missing or invalid; state was preserved.\n' >&2
      return 1
      ;;
  esac
  [ "$saved_port" -ge 1024 ] && [ "$saved_port" -le 65535 ] || {
    printf 'Recorded Aurora Skin injector port is out of range; state was preserved.\n' >&2
    return 1
  }
  if [ -z "$saved_start" ] || [ -z "$saved_node" ] || [ -z "$saved_injector" ]; then
    printf 'Recorded Aurora Skin injector identity is incomplete; state was preserved.\n' >&2
    return 1
  fi
  ...
  if recorded_injector_process_matches "$pid" "$saved_start" "$saved_node" "$saved_injector" "$saved_port"; then
    printf 'Could not stop the recorded Aurora Skin injector (PID %s).\n' "$pid" >&2
    return 1
  fi
  return 0
}
...
launch_injector_daemon() {
  ...
  # A short-lived launcher may terminate background children when it exits.
...
ensure_node_runtime() {
  if [ "$AURORA_SKIN_VALIDATED_RUNTIME_PID" = "$$" ] \
    && [ -n "$AURORA_SKIN_VALIDATED_RUNTIME_NODE" ] \
    && [ "${NODE:-}" = "$AURORA_SKIN_VALIDATED_RUNTIME_NODE" ] \
    && [ "${CODEX_BUNDLE:-}" = "$AURORA_SKIN_VALIDATED_RUNTIME_BUNDLE" ] \
    && [ "${CODEX_EXE:-}" = "$AURORA_SKIN_VALIDATED_RUNTIME_EXE" ]; then
    return 0
  fi
  discover_codex_app
  require_signed_node_runtime
...
# or after the launcher exits — that is unexpected and unwanted.
release_codex_launchd_job() {
