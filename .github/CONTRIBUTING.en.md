...
Thanks for contributing to Codex Aurora Skin. The project loads external themes into the official Codex desktop app through loopback CDP. macOS and Windows have separate install, injection, and restore paths, so choose the target platform before changing files.
...
For injection, CSS, launch, or restore changes, also run `macos/scripts/verify-aurora-skin-macos.sh` and inspect both the home and normal task routes.
...
For install, launch, injection, or restore changes, exercise the affected scripts and `windows/scripts/verify-aurora-skin.ps1`. Include the Windows version and Codex source in the pull request.
